...
onMounted(async () => {
  ...
  if (glucoseStore.readings.length === 0) {
    await glucoseStore.fetchReadings(
      authStore.user.uid,
    )
  }

  const reading = glucoseStore.getReadingById(
    route.params.id,
  )

  if (!reading) {
    alert('Mjerenje nije pronađeno.')
    router.push('/glucose')
    return
  }
  ...
  measurementType.value = reading.measurementType
  note.value = reading.note || ''

  if (reading.measuredAtDate) {
    measuredAt.value = formatForInput(
...
async function updateReading() {
  ...
  await glucoseStore.updateReading(
    route.params.id,
    {
  ...
  <DashboardLayout>
    <div class="max-w-xl">
      ...
      <p class="mb-6 text-gray-500">
        Promijenite podatke odabranog mjerenja.
      </p>

      <form
        class="flex flex-col gap-4 rounded-lg bg-white p-6"
        @submit.prevent="updateReading"
      >
        <div
